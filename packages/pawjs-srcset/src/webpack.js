      ...
      try {
        ...
        conf.forEach(c => {
          ...
          moduleRules.forEach((imageRule, imageRuleIndex) => {
            ...
            if (JSON.stringify(imageRule).indexOf("pwa-srcset-loader") !== -1) return false;
            
            // Do not do anything if { use: ... } is not found inside the image rule
            if (!imageRule.use) return false;
            
            const ruleUse = typeof imageRule.use === "string" ? imageRule.use: (
              typeof imageRule.use === "object" ? JSON.parse(JSON.stringify(imageRule.use)): imageRule.use
            );
            
            const ruleUseClone = typeof imageRule.use === "string" ? imageRule.use: (
              typeof imageRule.use === "object" ? JSON.parse(JSON.stringify(imageRule.use)): imageRule.use
            );
