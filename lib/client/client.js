...
const customClient = require(`${process.env.__p_root}src/client`);
...
// Custom reducers
let { reduxEnhancers, reduxMiddlewares, reduxReducers, reduxInitialState, onPageChange } = customClient;
...
global.store = global.store || configureStore({
  ...
  ...(reduxMiddlewares ? { middleware: reduxMiddlewares} : {}),
  ...(reduxEnhancers ? { enhancers: reduxEnhancers} : {}),
