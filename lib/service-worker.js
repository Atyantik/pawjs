...
workboxSW
  .router
  .registerRoute(
    new RegExp("^"+ self.location.origin +"/.(css|bmp|tif|ttf|docx|woff2|js|pict|tiff|eot|xlsx|jpg|csv|eps|woff|xls|jpeg|doc|ejs|otf|pptx|gif|pdf|swf|ps|ico|pls|midi|svgz|class|png|ppt|mid|webp|jar)$", "g"),
...
workboxSW
  .router
  .registerRoute(
    self.location.origin + "/_globals",
    workboxSW.strategies.cacheFirst()
  );

workboxSW.router.registerRoute(new RegExp("^"+ self.location.origin +"/.*"),  ({event}) => {
