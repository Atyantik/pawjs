...
import {getPreloadDataPromises, renderNotFoundPage, renderRoutesByUrl} from "../utils/renderer";
...
import {applyServerFilter, getErrorComponent} from "../utils/server";
import Wook from "wook";

const DefaultRoutes = require(`${process.env.__p_root}src/routes`).default;
const assets = require(`${process.env.__p_root}src/config/assets`).default;
const config = require(`${process.env.__p_root}src/config`).default;
const pwaIcon = require(`${process.env.__p_root}src/resources/images/pwa/icon-384x384.png`);

const { publicDirName } = require(`${process.env.__p_root}directories`);
...
const getAllAssets = () => {
  if (_.isEmpty(allAssets)) {
    ...
    allAssets = _.filter(
      _.map(allAssets, a => {
        let replacedPath = a.replace(publicDirPrefix, "");

        // Path for windows!
        if (path.sep === "\\") {
          let publicDirPrefixWin = publicDirPrefix.replace(/\\/g, "/");
...
app.get("*", async (req, res, next) => {
  ...
  const pawHead = await applyServerFilter(res, "paw_head", [], {req, res});
  const pawFooter = await applyServerFilter(res, "paw_footer", [], {req, res});
  ...
  try {
    ...
    html = ReactDOMServer.renderToString((
      <Html
        ...
        url={currentUrl}
        isBot={bot}
      >{routerComponent}</Html>
    ));
    res.status(err.statusCode || 500).send(`<!DOCTYPE html>${html}`);
  }
  next();
});
